...
async function recognizeCanvasText(canvas: HTMLCanvasElement): Promise<string> {
  ...
  try {
    ...
    const preparedBase = cropCanvasToContent(enhanceCanvasForOCR(cloneCanvas(canvas)));
    for (const angle of OCR_ROTATIONS) {
      const candidate = cropCanvasToContent(rotateCanvas(preparedBase, angle));
      ...
      if (hasUsableText(text) && /CPF|RENAVAM|PLACA|ENDERE|MODELO|REGISTRO|NOME/.test(text.toUpperCase())) {
        break;
      }
    }
...
function extractValueAfterLabel(lines: string[], labelPattern: RegExp): string {
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (sameLine && !looksLikeVehicleLabelLine(sameLine)) {
      return sameLine;
    }
...
function parseComprovanteText(text: string): Partial<ExtractedDocumentFields> {
  ...
  const cidadeUfLine =
    lines.find((line) => /\b[A-ZÀ-Ú]{3,}(\s+[A-ZÀ-Ú]+)*\s*[-\/]?\s*[A-Z]{2}\b/.test(line) && !looksLikeAddressLine(line) && !/CEP|CNPJ|CPF|FONE|TELEFONE|CELULAR/.test(line) && !isUtilityNoiseLine(line)) || "";
  const cidadeUfMatch = cidadeUfLine.match(/([A-ZÀ-Ú][A-ZÀ-Ú ]+?)\s*[-\/]?\s*\b([A-Z]{2})\b/);
  const uf = cidadeUfMatch?.[2] || "";
...
function parseCRLVText(text: string): Partial<ExtractedDocumentFields> {
  ...
  const corFromLabel = extractValueAfterLabel(lines, /\bCOR(?:\s+PREDOMINANTE)?\b/);
  const cor = findFirstGroup(corFromLabel || upper, [
    /(?:COR(?:\s+PREDOMINANTE)?)[^A-ZÀ-Ú]*([A-ZÀ-Ú ]{3,})/,
    /\b(PRETA|BRANCA|PRATA|CINZA|VERMELHA|AZUL|VERDE|AMARELA|LARANJA|ROXA|MARROM)\b/,
  ]);

  const anoFabLine = extractValueAfterLabel(lines, /\bANO\s+FABRICA[ÇC][AÃ]O\b/);
  const anoModeloLine = extractValueAfterLabel(lines, /\bANO\s+MODELO\b/);
  const anoComposto = [anoFabLine, anoModeloLine].filter(Boolean).join("/");
  const ano = findFirstGroup(anoComposto || upper, [/\b([0-9]{4}\/[0-9]{4})\b/]);

  const marcaModeloRaw =
    extractValueAfterLabel(lines, /\bMARCA\s*\/\s*MODELO(?:\s*\/\s*VERS[ÃA]O)?\b/) ||
    findFirstGroup(upper, [/(?:MARCA\s*\/\s*MODELO(?:\s*\/\s*VERS[ÃA]O)?|MODELO)[^A-Z0-9]*([A-Z0-9/ .\-]{6,})/]);
  ...
  const anoFinal = /[0-9]{4}\/[0-9]{4}/.test(ano)
    ? ano.match(/[0-9]{4}\/[0-9]{4}/)?.[0] || ""
    : [anoFabLine, anoModeloLine].filter(Boolean).join("/");
